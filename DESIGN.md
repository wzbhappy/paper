...
第一版（v0.1）Phase 1–4 均已完成。以下记录各期的实际交付与验收情况。

### Phase 1 — 单链路跑通 ✅
- 项目/文献基础 CRUD + PostgreSQL schema
- PDF 上传 → 解析 → 结构化摘要 → 入向量库
- 「文献 → 研究方向建议」一条链路
- 基础前端：文献库 + 方向卡片
- **交付说明**：解析主实现用 pymupdf（按字号推断标题层级），marker-pdf 作为可选后端，安装后自动优先使用并在失败时回退。方向建议强制引用库内文献，无证据的建议被丢弃。

### Phase 2 — 检索 + 综述闭环 ✅
- 多源 Retriever（arXiv + Semantic Scholar + Crossref）
- 跨源去重 + 一键入库
- 引用图谱 + 综述生成
- **交付说明**：社区发现用标签传播而非 Louvain，迭代顺序确定性可复现；边少于 3 条时退化为连通分量。综述引用越界会被剥离并计数上报。

### Phase 3 — 写作链路 ✅
- 大纲生成 + 4 套模板库
- 正文编辑器 + 7 种 AI 写作动作
- 引用插入与一致性校验
- 多格式导出（Markdown / LaTeX / Word / BibTeX）
- **交付说明**：编辑器为 Markdown textarea 而非 TipTap（决策见第 13 节）。纯语言加工动作不传入文献以避免模型硬塞引用。

### Phase 4 — 全流程 + 润色 + 导出 ✅
- 热点分析模块（补齐①）
- 完整质量检查面板（引用/术语/查重/格式/语言）
- 阶段进度引导串联全流程
- **交付说明**：质量检查为纯规则实现，无 LLM 调用。热点分析的趋势窗口相对文献库最新年份计算。
...
- 后端 341 个测试，全部不依赖外部服务（LLM 用 fake provider，HTTP 用 MockTransport，向量库与图谱用内存实现）
- 四个端到端测试分别覆盖各期主链路
- 未覆盖：Neo4j / Qdrant / 真实 LLM API 的集成测试；三个检索源真实响应格式的变化
...
- [x] 图数据库是否引入 Neo4j → **引入 Neo4j**，同时提供内存实现供测试与降级
- [x] 富文本导出格式优先级 → **Markdown > LaTeX > Word**，四种格式均已实现
- [x] Phase 3 富文本编辑器 → **暂用 Markdown textarea**。核心需求是「选中文字 → AI 加工 → 替换回原位」，textarea 的 selectionStart/End 已足够，且便于直接编辑引用标记。TipTap 需引入多个包，收益主要在所见即所得与公式渲染，留待后续评估。
- [x] 热点分析的近期窗口 → **相对文献库最新年份**而非系统当前时间，避免旧文献库中所有关键词都被判为过时
- [x] 质量检查是否用 LLM → **纯规则实现**。规则检查可解释、可重复、零成本，适合作为写作过程中的高频反馈；LLM 式的语义审阅留待后续作为可选增强

### 后续待定

- [ ] 是否引入 TipTap 富文本编辑器（需要公式渲染与行内引用气泡时）
- [ ] 中文文献源（知网/万方）接入方式与授权
- [ ] 是否做 VS Code 插件形态
- [ ] 术语一致性检查支持项目级自定义术语表
- [ ] 查重是否扩展为与向量库中的外部文献比对
